...
本分支包含 iOS Bootstrap, Framework 源码, CN 客户端处理脚本和 macOS 构建入口. Windows 构建机通过 SSH 调用 macOS 编译 Framework, 之后在 Windows 生成 unsigned IPA.

设备签名在 macOS 上执行:

```sh
platforms/ios/sign-device-ipa.sh \
  --development \
  --input /path/to/unsigned.ipa \
  --output /path/to/signed.ipa \
  --bundle-id dev.example.starpoint \
  --identity "Apple Development: Example" \
  --profile /path/to/profile.mobileprovision
```

证书, 私钥, profile 和输出包保持在仓库外.
