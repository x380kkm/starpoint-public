...
本分支包含 Android CompanionHost, ARM64 JNI bridge, CN 客户端处理脚本和完整 APK 合包入口. Android 只使用项目专用的无头 AVD 进行验证, ADB 脚本固定目标 serial, 不接管其他设备.
...
APK, keystore, 密码文件, AVD 数据和 ADB 日志保持在仓库外.
