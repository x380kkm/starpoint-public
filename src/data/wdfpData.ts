...
import referencePlayerData from "../../assets/save_data.json";

const db = getDatabase(Database.WDFP_DATA)
const expPoolMax = 100000

const DAILY_WEEK_QUEST_CATEGORY = 6
// //// 生成默认每日关卡随机表 [@x380kkm 2026-08-27] ////
function getDefaultDailyDrawnQuests(existing: PlayerDrawnQuest[]): PlayerDrawnQuest[] {
    const existingDailyWeekOdds = new Map(
        existing
            .filter((quest) => quest.categoryId === DAILY_WEEK_QUEST_CATEGORY)
            .map((quest) => [quest.questId, quest.oddsId]),
    )
    const dailyWeekDrawnQuests = referencePlayerData.drawn_quest_list
        .filter((quest) => quest.category_id === DAILY_WEEK_QUEST_CATEGORY)
        .map((quest) => ({
            categoryId: DAILY_WEEK_QUEST_CATEGORY,
            questId: quest.quest_id,
            oddsId: existingDailyWeekOdds.get(quest.quest_id) ?? quest.odds_id,
        }))
    return dailyWeekDrawnQuests.concat(
        existing.filter((quest) => quest.categoryId !== DAILY_WEEK_QUEST_CATEGORY),
    )
}
// //// /生成默认每日关卡随机表 ////

// //// 在游戏数据库事务中执行同步状态变更 [@x380kkm 2026-08-04] ////
export function runPlayerDataTransactionSync<Result>(operation: () => Result): Result {
...
// //// 分页读取账号并返回总数 [@x380kkm 2026-07-22] ////
...
function listAccountsPageSync(request: AccountPageRequest): AccountPage {
    ...
    ORDER BY id
    LIMIT ? OFFSET ?
    `).all(request.limit, request.offset) as RawAccount[]
    const count = db.prepare("SELECT COUNT(*) AS total FROM accounts").get() as { total: number }

    return { accounts: rawAccounts.map(buildAccount), total: count.total }
}

export function listAccountsPage(request: AccountPageRequest): Promise<AccountPage> {
    return new Promise<AccountPage>((resolve, reject) => {
        try {
            resolve(listAccountsPageSync(request))
        } catch (error) {
            reject(error)
        }
    })
...
export function getAccountPlayersSync(
    ...
    SELECT players.id
    FROM players
    LEFT JOIN account_active_players
        ON account_active_players.account_id = players.account_id
    WHERE players.account_id = ?
    ORDER BY
        CASE WHEN players.id = account_active_players.player_id THEN 0 ELSE 1 END,
        players.id
...
// //// 列出全部玩家存档 ID [@x380kkm 2026-07-27] ////
export function getAllPlayerIdsSync(): number[] {
...
// //// 读取并切换账号的激活存档 [@x380kkm 2026-07-27] ////
export function getActivePlayerIdSync(
    accountId: number
): number | null {
    const raw = db.prepare(`
    SELECT player_id
    FROM account_active_players
    WHERE account_id = ?
    `).get(accountId) as { player_id: number } | undefined

    return raw?.player_id ?? null
}
...
export function getActivePlayerId(
    ...
    return new Promise<number | null>((resolve, reject) => {
        try {
            resolve(getActivePlayerIdSync(accountId))
        } catch (error) {
            reject(error)
        }
    })
...
export function getActivePlayerSync(
    accountId: number
): Player | null {
    const playerId = getActivePlayerIdSync(accountId)
    return playerId === null ? null : getPlayerSync(playerId)
}

export function getActivePlayer(
    accountId: number
): Promise<Player | null> {
    return new Promise<Player | null>((resolve, reject) => {
        try {
            resolve(getActivePlayerSync(accountId))
        } catch (error) {
            reject(error)
        }
    })
...
const activateAccountPlayerTransaction = db.transaction((
    accountId: number,
    playerId: number
): Player => {
    const ownedPlayer = db.prepare(`
    SELECT id
    FROM players
    WHERE account_id = ? AND id = ?
    `).get(accountId, playerId) as { id: number } | undefined

    if (ownedPlayer === undefined) {
        throw new Error("Account does not own the requested player.")
    }

    db.prepare(`
    INSERT INTO account_active_players (account_id, player_id)
    VALUES (?, ?)
    ON CONFLICT(account_id) DO UPDATE SET player_id = excluded.player_id
    `).run(accountId, playerId)

    const player = getPlayerSync(playerId)
    if (player === null) throw new Error("Activated player does not exist.")
    return player
})

export function activateAccountPlayerSync(
    accountId: number,
    playerId: number
): Player {
    return activateAccountPlayerTransaction(accountId, playerId)
}

export function activateAccountPlayer(
    accountId: number,
    playerId: number
): Promise<Player> {
    return new Promise<Player>((resolve, reject) => {
        try {
            resolve(activateAccountPlayerSync(accountId, playerId))
        } catch (error) {
            reject(error)
        }
    })
...
function setInitialActivePlayerSync(
    accountId: number,
    playerId: number
): void {
    const result = db.prepare(`
    INSERT OR IGNORE INTO account_active_players (account_id, player_id)
    SELECT account_id, id
    FROM players
    WHERE account_id = ? AND id = ?
    `).run(accountId, playerId)

    if (result.changes === 0 && getActivePlayerIdSync(accountId) === null) {
        throw new Error("Account does not own the initial player.")
    }
}
// //// /读取并切换账号的激活存档 ////
...
// //// 读取账号的全部会话供控制面生成脱敏摘要 [@x380kkm 2026-07-22] ////
function getAccountSessionsSync(accountId: number): Session[] {
    const rawSessions = db.prepare(`
    SELECT token, account_id, expires, type
    FROM sessions
    WHERE account_id = ?
    ORDER BY type, expires
    `).all(accountId) as RawSession[]

    return rawSessions.map(buildSession)
}

export function getAccountSessions(accountId: number): Promise<Session[]> {
    return new Promise<Session[]>((resolve, reject) => {
        try {
            resolve(getAccountSessionsSync(accountId))
        } catch (error) {
            reject(error)
        }
    })
...
export function deleteAccountSessions(
    ...
    return new Promise<void>((resolve, reject) => {
        try {
            resolve(deleteAccountSessionsSync(accountId))
        } catch (error) {
            reject(error)
        }
    })
...
export function getPlayerMissionCountersSync(
    playerId: number
): Record<string, number> {
    const rows = db.prepare(`
    ...
    WHERE player_id = ?
    `).all(playerId) as RawPlayerMissionCounter[]

    return Object.fromEntries(rows.map(row => [row.pattern, row.value]))
}

export function addPlayerMissionCountersSync(
    playerId: number,
    counters: Record<string, number>
) {
    const statement = db.prepare(`
    INSERT INTO players_mission_counters (pattern, value, player_id)
    VALUES (?, ?, ?)
    ON CONFLICT(pattern, player_id) DO UPDATE SET value = value + excluded.value
    `)
    db.transaction(() => {
        for (const [pattern, value] of Object.entries(counters)) {
            statement.run(pattern, value, playerId)
        }
    })()
}
...
export function getPlayerFromAccountIdSync(
    ...
    return getActivePlayerSync(accountId)
}
 ...
 */
function insertMergedPlayerRelationsSync(
    playerId: number,
...
export function insertMergedPlayerDataSync(
    ...
    const playerId = insertPlayerSync(accountId, toInsert.player)
    insertMergedPlayerRelationsSync(playerId, toInsert)
    return playerId
}

// //// 将导入数据原子写入新的玩家存档 [@x380kkm 2026-07-27] ////
const importPlayerDataAsNewSlotTransaction = db.transaction((
    accountId: number,
    toImport: MergedPlayerData
): Player => {
    if (getAccountSync(accountId) === null) {
        throw new Error("Account does not exist.")
    }

    const playerWithoutSourceId = {
        ...toImport.player,
        id: undefined
    }
    const playerId = insertPlayerSync(accountId, playerWithoutSourceId)
    insertMergedPlayerRelationsSync(playerId, toImport)
    setInitialActivePlayerSync(accountId, playerId)

    const player = getPlayerSync(playerId)
    if (player === null) throw new Error("Imported player does not exist.")
    return player
})

export function importPlayerDataAsNewSlotSync(
    accountId: number,
    toImport: MergedPlayerData
): Player {
    return importPlayerDataAsNewSlotTransaction(accountId, toImport)
}

export function importPlayerDataAsNewSlot(
    accountId: number,
    toImport: MergedPlayerData
): Promise<Player> {
    return new Promise<Player>((resolve, reject) => {
        try {
            resolve(importPlayerDataAsNewSlotSync(accountId, toImport))
        } catch (error) {
            reject(error)
        }
    })
...
export function insertDefaultPlayerSync(
    ...
    insertPlayerDrawnQuestsSync(playerId, getDefaultDailyDrawnQuests([
        {
            categoryId: 6,
            questId: 5001,
            oddsId: 5
 ...
 */
// //// 在一个 SQLite 事务内替换完整玩家存档 [@x380kkm 2026-07-22] ////
function deletePlayerRecordSync(playerId: number): void {
    db.prepare("DELETE FROM players WHERE id = ?").run(playerId)
}

const replacePlayerDataTransaction = db.transaction((replaceWith: MergedPlayerData) => {
    ...
    const wasActive = getActivePlayerIdSync(account.id) === playerId
    db.prepare("INSERT INTO player_data_replacement_guards (player_id) VALUES (?)").run(playerId)
    deletePlayerRecordSync(playerId)
    insertMergedPlayerDataSync(account.id, replaceWith)
    db.prepare("DELETE FROM player_data_replacement_guards WHERE player_id = ?").run(playerId)
    if (wasActive) activateAccountPlayerSync(account.id, playerId)
})

export function replacePlayerDataSync(replaceWith: MergedPlayerData): void {
    replacePlayerDataTransaction(replaceWith)
}
// //// /在一个 SQLite 事务内替换完整玩家存档 ////

// //// 按服务器时间结算玩家的池化经验 [@x380kkm 2026-08-03] ////
export function collectPlayerDataPooledExpSync(
    player: Player,
    dateNow?: Date
) {
    const effectiveDateNow = dateNow ?? getServerDate()
    const serverTimeNow = getServerTime(effectiveDateNow)
    ...
    updatePlayerSync({
        id: player.id,
        expPooledTime: effectiveDateNow,
