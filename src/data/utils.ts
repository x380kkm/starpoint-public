...
// # player-data-codec
//
// 该模块在 SQLite 玩家模型, CN 客户端数据和可移植数据之间转换.
// 客户端数据保留目标实例字段, 可移植数据删除实例身份字段.

import { getCharacterDataSync } from "../lib/assets"
import { findTutorialGachaCharacterId } from "../lib/tutorial"
...
import {
    getPortablePlayerSnapshotSync,
    setPortablePlayerSnapshotSync,
} from "./portablePlayerSnapshots"
import {
    mergePortableExtrasIntoClientData,
    reconcilePortableGameData,
} from "../games/starpoint/portablePlayerData"
import { sanitizePortableGameData } from "../games/starpoint/portableSave"
...
export function serializePlayerData(
    ...
    const tutorialGachaCharacterId = findTutorialGachaCharacterId(Object.keys(userCharacterList))
    ...
    const storedTutorialStep = playerData.tutorialStep
    const tutorialGachaResultStep = playerData.tutorialSkipFlag ? 4 : 15
    const tutorialStep = storedTutorialStep === tutorialGachaResultStep && tutorialGachaCharacterId === null
        ? tutorialGachaResultStep - 1
        : storedTutorialStep
    ...
    const clientData: ClientPlayerData = {
        ...
        "tutorial_gacha": tutorialGachaCharacterId === null ? null : {
            "character_id": tutorialGachaCharacterId,
        },
...
// //// 生成建模数据并合并可移植来源快照 [@x380kkm 2026-08-03] ////
/**
 * 读取服务器已建模的客户端数据.
 *
 * @param playerId 玩家槽 ID.
 * @param options 序列化选项.
 * @returns 客户端数据, 不存在时返回 null.
 */
export function getModeledClientSerializedData(
    playerId: number,
    options: SerializePlayerDataOptions
...
export function getClientSerializedData(
    ...
    const current = getModeledClientSerializedData(playerId, options)
    if (current === null) return null
    const snapshot = getPortablePlayerSnapshotSync(playerId)
    if (snapshot === null) return current
    return mergePortableExtrasIntoClientData(
        snapshot.source,
        snapshot.baseline,
        current as unknown as Record<string, unknown>,
    ) as unknown as ClientPlayerData
}

export function getPortableSerializedData(
    playerId: number,
    options: SerializePlayerDataOptions,
    ...
    if (current === null) return null
    const snapshot = getPortablePlayerSnapshotSync(playerId)
    const portableData = snapshot === null
        ? current as unknown as Record<string, unknown>
        : reconcilePortableGameData(
            snapshot.source,
            snapshot.baseline,
    ...
    return sanitizePortableGameData(portableData)
}

export function capturePortablePlayerSnapshotSync(
    playerId: number,
    source: Record<string, unknown>,
): void {
    const baseline = getModeledClientSerializedData(
        playerId,
        { serializeRushEventData: true, viewerId: 0 },
    )
    if (baseline === null) throw new Error("Imported player baseline is unavailable.")
    setPortablePlayerSnapshotSync(playerId, {
        source: sanitizePortableGameData(source),
