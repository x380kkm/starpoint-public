...
import { commitSaveRevisionChangeSync, createSaveRevisionSync } from "../../data/saveRevisions";
...
const routes = async (fastify: FastifyInstance) => {
    ...
    fastify.post("/save", async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        try {
            ...
            const currentData = getClientSerializedData(playerId, { serializeRushEventData: true, viewerId: 0 })
            if (currentData === null) return reply.redirect(`/player/${id}?error=save_not_found`);
            commitSaveRevisionChangeSync(() => {
                createSaveRevisionSync({ playerId, data: currentData, label: "Before overwrite" })
                replacePlayerDataSync(parsedData)
                const importedData = getClientSerializedData(playerId, { serializeRushEventData: true, viewerId: 0 })
                if (importedData === null) throw new Error("Imported save does not exist.")
                createSaveRevisionSync({ playerId, data: importedData, label: "Legacy web overwrite" })
            })
