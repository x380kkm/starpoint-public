// audience: internal
// # shop-routes
// 此模块实现商店购买, 销售列表和体力恢复路由.
// Fastify 将路由注册到 /api/index.php/shop, 运行前需初始化玩家和资产数据存储.
// 资源变更在账户和余额校验通过后使用同步数据库更新保存.
...
// //// 恢复玩家体力并扣除星导石 [@x380kkm 2026-08-04] ////
interface RecoverStaminaBody {
    api_count: number
    viewer_id: number
}

interface VmoneyBalance {
    freeVmoney: number
    vmoney: number
}

const staminaRecoveryCost = 50
const staminaRecoveryAmount = 100
const staminaOverflowLimit = 999

function spendVmoney(freeVmoney: number, vmoney: number, cost: number): VmoneyBalance | null {
    const freeVmoneySpent = Math.min(freeVmoney, cost)
    const vmoneySpent = cost - freeVmoneySpent
    if (vmoney < vmoneySpent) return null

    return {
        freeVmoney: freeVmoney - freeVmoneySpent,
        vmoney: vmoney - vmoneySpent,
    }
}

function registerStaminaRecoveryRoute(fastify: FastifyInstance): void {
    fastify.post("/recover_stamina", async (request: FastifyRequest, reply: FastifyReply) => {
        const body = request.body as Partial<RecoverStaminaBody>
        const viewerId = body.viewer_id
        if (typeof viewerId !== "number" || !Number.isInteger(viewerId)) {
        ...
        const player = playerId === undefined ? null : getPlayerSync(playerId)
        if (player === null) {
        ...
        const currentStamina = Math.max(0, player.stamina)
        const recoveredStamina = Math.min(currentStamina + staminaRecoveryAmount, staminaOverflowLimit)
        if (recoveredStamina === currentStamina) {
        ...
        const balance = spendVmoney(player.freeVmoney, player.vmoney, staminaRecoveryCost)
        if (balance === null) {
            return reply.status(400).send({
                "error": "Bad Request",
                "message": "Not enough star guide stones.",
            })
        }

        const staminaHealTime = getServerDate()
        updatePlayerSync({
            id: playerId,
            stamina: recoveredStamina,
            staminaHealTime,
            freeVmoney: balance.freeVmoney,
            vmoney: balance.vmoney,
        })
        ...
        return reply.status(200).send({
            ...
            "data": {
                "user_info": {
                    "stamina": recoveredStamina,
                    "stamina_heal_time": getServerTime(staminaHealTime),
                    "free_vmoney": balance.freeVmoney,
                    "vmoney": balance.vmoney,
                },
            },
        })
...
const routes = async (fastify: FastifyInstance) => {
    registerStaminaRecoveryRoute(fastify)
