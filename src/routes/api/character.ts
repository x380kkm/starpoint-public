...
const routes = async (fastify: FastifyInstance) => {
    ...
    fastify.post("/over_limit", async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        if (!Number.isSafeInteger(body.over_limit_count) || body.over_limit_count <= 0) return reply.status(400).send({
            "error": "Bad Request",
            "message": "Invalid over limit count."
        })
        if (typeof body.use_stack !== "boolean") return reply.status(400).send({
            "error": "Bad Request",
            "message": "Invalid stack usage flag."
        })
        ...
        if (body.use_stack) {
            ...
            if (!Number.isSafeInteger(itemId) || itemId <= 0) return reply.status(400).send({
                "error": "Bad Request",
                "message": "Invalid uncapping item."
            })
