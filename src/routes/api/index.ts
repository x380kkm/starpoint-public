...
const routes = async (fastify: FastifyInstance) => {
    // //// 验证双凭证归属同一账号后载入玩家数据 [@x380kkm 2026-07-20] ////
    fastify.post("/load", async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        if (session.accountId !== viewerSession.accountId) return reply.status(400).send({
            "error": "Bad Request",
            "message": "Credentials belong to different accounts."
        })
