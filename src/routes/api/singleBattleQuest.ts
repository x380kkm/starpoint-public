// audience: external
// # battle-quest-routes
// 此模块实现单人战斗流程, 并向多人前缀复用同一套真实结算和奖励逻辑.
// 结算和中止只消费与当前 quest 和 play_id 一致的一次性状态.

import { FastifyInstance, FastifyPluginOptions, FastifyReply, FastifyRequest } from "fastify";
import { deleteActiveQuest, getActiveQuest, insertActiveQuest } from "../../control/battleSettlementState";
...
export interface BattleQuestRouteOptions extends FastifyPluginOptions {
    battleGroup?: "single" | "multi"
}

const continueVmoneyCost = 50;

export { deleteActiveQuest, insertActiveQuest } from "../../control/battleSettlementState"

// //// 注册可复用的战斗结算和中止路由 [@x380kkm 2026-07-24] ////
export const registerBattleSettlementRoutes = async (
    fastify: FastifyInstance,
    options: BattleQuestRouteOptions = {},
) => {
    const battleGroup = options.battleGroup ?? "single"
    ...
    fastify.post("/finish", async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        const activeQuestData = getActiveQuest(playerId)
        if (activeQuestData === null) return reply.status(400).send({
            "error": "Bad Request",
            "message": "No active quest to finish."
        })
        if (body.play_id !== activeQuestData.playId) return reply.status(409).send({
            "error": "Conflict",
            "message": "The play id does not match the active quest."
        })
        if (body.quest_id !== activeQuestData.questId || body.category !== activeQuestData.category) {
            return reply.status(409).send({
                "error": "Conflict",
                "message": "The quest does not match the active quest."
            })
        }
    ...
    fastify.post("/abort", async (request: FastifyRequest, reply: FastifyReply) => {
        ...
        const activeQuest = getActiveQuest(playerId)
        if (activeQuest !== null && activeQuest.playId !== body.play_id) return reply.status(409).send({
            "error": "Conflict",
            "message": "The play id does not match the active quest."
        })
        if (activeQuest !== null && (body.quest_id !== activeQuest.questId || body.category !== activeQuest.category)) {
            return reply.status(409).send({
                "error": "Conflict",
                "message": "The quest does not match the active quest."
            })
...
// //// 注册单人战斗开始和继续路由 [@x380kkm 2026-07-24] ////
const registerSingleBattleQuestRoutes = async (fastify: FastifyInstance, options: BattleQuestRouteOptions = {}) => {
    await registerBattleSettlementRoutes(fastify, options)
...
export default registerSingleBattleQuestRoutes;
