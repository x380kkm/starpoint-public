...
function Test-StaticAdbTimeoutCoverage {
    ...
    Assert-TestCondition -Condition ($DirectAdbCalls.Count -eq 4) -Message "启动器的直接 ADB 调用数量已变化, 请更新超时覆盖测试"
...
function Test-AdbServerPortContract {
    ...
    Assert-TestCondition -Condition ($StartPort.Count -eq 1 -and $StartPort[0].Extent.Text -match '\[Nullable\[int\]\]\$AdbServerPort\b' -and $StartPort[0].Extent.Text -notmatch '=') -Message "启动器 AdbServerPort 必须可空以便使用环境端口"
    ...
    try {
        ...
        Assert-TestCondition -Condition ($DefaultPort -eq 5037) -Message "缺少参数, 状态和环境时未回退到 5037"
        Assert-TestCondition -Condition ($EnvironmentPort -eq 5038) -Message "旧状态未使用 ANDROID_ADB_SERVER_PORT"
        ...
        [Environment]::SetEnvironmentVariable("ANDROID_ADB_SERVER_PORT", "invalid", [EnvironmentVariableTarget]::Process)
        $InvalidEnvironmentFailed = $false
        try {
            Resolve-ProtocolLabAdbServerPort -State ([pscustomobject]@{}) | Out-Null
        } catch {
            $InvalidEnvironmentFailed = $_.Exception.Message -match "ANDROID_ADB_SERVER_PORT 无效"
        }
        Assert-TestCondition -Condition $InvalidEnvironmentFailed -Message "非法 ANDROID_ADB_SERVER_PORT 未明确失败"
...
function Invoke-StartEmulatorWithMocks {
    ...
    function Resolve-ProtocolLabAdbServerPort {
        ...
        $EnvironmentPort = [Environment]::GetEnvironmentVariable("ANDROID_ADB_SERVER_PORT", [EnvironmentVariableTarget]::Process)
...
function Test-StartEmulatorAdbServerPortForwarding {
    ...
    try {
        ...
        Assert-AdbServerPortRun -Run $DefaultRun -ExpectedPort 5037
        ...
        Assert-AdbServerPortRun -Run $EnvironmentRun -ExpectedPort 5038
...
function Test-StartEmulatorTimeoutForwarding {
    ...
    $StartupRecords = @(
        "Invoke-ProtocolLabAdb:get-state",
        "Wait-ProtocolLabAndroidBoot",
        "Enable-ProtocolLabCaptureRoute",
    ...
    $DefaultRun = Invoke-StartEmulatorWithMocks -StartEmulatorPath $StartEmulatorPath -AdbCommandTimeoutSeconds 5 -AdbServerPort 5037 -UseDefaultTimeout
    ...
    $CustomRun = Invoke-StartEmulatorWithMocks -StartEmulatorPath $StartEmulatorPath -AdbCommandTimeoutSeconds 9 -AdbServerPort 5037
    ...
    $CleanupRun = Invoke-StartEmulatorWithMocks -StartEmulatorPath $StartEmulatorPath -AdbCommandTimeoutSeconds 7 -AdbServerPort 5037 -FailDuringCaptureValidation
    ...
    Assert-TimeoutRecords -Records $CleanupRun.TimeoutRecords -ExpectedNames ($StartupRecords + @("Restore-ProtocolLabProxy", "Invoke-ProtocolLabAdb:emu kill")) -ExpectedTimeoutSeconds 7
}
