...
# 此脚本在临时夹具和真实 SWF 上验证正式登录路径, 登录请求探测, 版本查询, 响应解码和预展开资源补丁只修改声明过的方法.
...
function Import-TestPatchFunctions {
    ...
    $FunctionNames = @(
        ...
        "Assert-ClientPatchSdkDummyPcode",
...
# //// 验证 SDK 登录补丁模式 [@x380kkm 2026-09-02] ////
...
try {
    $ProductionMode = Get-ClientPatchSdkLoginMode -EmbeddedPersonalService $true -ProbeSdkDummyLoginRequest $false
    Assert-TestCondition -Condition ($ProductionMode.Name -ceq "embedded-personal-service") -Message "内嵌个人服务登录模式名称不正确"
    Assert-TestCondition -Condition ($ProductionMode.EmbeddedPersonalService -and $ProductionMode.PatchApiEndpoint -and $ProductionMode.PatchSdkDummy -and $ProductionMode.PatchChannelMainForSdkDummy -and -not $ProductionMode.PatchDummyLoginRequestProbe -and $ProductionMode.PatchSdkDummyNativeExtensionGuard -and $ProductionMode.PatchRemoteUtilRequestGuard -and $ProductionMode.PatchSdkDummyTitleSceneGuard) -Message "内嵌个人服务没有原子启用正式登录补丁"
    ...
    Assert-TestCondition -Condition ($SdkDummyResult.Changed -and $SdkDummyResult.To -ceq "true" -and [IO.File]::ReadAllText($DevConfigPath).Contains("sdkDummy:Boolean = true", [StringComparison]::Ordinal)) -Message "正式登录补丁没有启用 sdkDummy"

    $ValidPcodePath = Join-Path $TemporaryDirectory "DevConfig-valid.pcode"
    $ValidPcode = @'
getlex QName(PackageNamespace("pinball.config.core"),"DevConfig")
pushtrue
initproperty QName(PackageNamespace(""),"sdkDummy")
'@
    [IO.File]::WriteAllText($ValidPcodePath, $ValidPcode, [Text.UTF8Encoding]::new($false))
    $PcodeVerification = Assert-ClientPatchSdkDummyPcode -PcodePath $ValidPcodePath
    Assert-TestCondition -Condition ($PcodeVerification.Verified -and $PcodeVerification.Instruction -ceq "pushtrue") -Message "最终 P-code 验证没有接受 sdkDummy=true"

    $InvalidPcodePath = Join-Path $TemporaryDirectory "DevConfig-invalid.pcode"
    [IO.File]::WriteAllText($InvalidPcodePath, $ValidPcode.Replace("pushtrue", "pushfalse", [StringComparison]::Ordinal), [Text.UTF8Encoding]::new($false))
    $InvalidPcodeRejected = $false
    try {
        Assert-ClientPatchSdkDummyPcode -PcodePath $InvalidPcodePath | Out-Null
    } catch {
        $InvalidPcodeRejected = $true
    }
    Assert-TestCondition -Condition $InvalidPcodeRejected -Message "最终 P-code 验证接受了 sdkDummy=false"
    ...
    $DefaultMode = Get-ClientPatchSdkLoginMode -EmbeddedPersonalService $false -ProbeSdkDummyLoginRequest $false
    Assert-TestCondition -Condition ($DefaultMode.Name -ceq "none" -and $DefaultMode.PatchApiEndpoint -and -not $DefaultMode.PatchSdkDummy -and -not $DefaultMode.PatchChannelMainForSdkDummy -and -not $DefaultMode.PatchDummyLoginRequestProbe -and -not $DefaultMode.PatchSdkDummyNativeExtensionGuard -and -not $DefaultMode.PatchRemoteUtilRequestGuard -and -not $DefaultMode.PatchSdkDummyTitleSceneGuard) -Message "外部服务模式的登录补丁范围不正确"
    ...
    $ProductionSelectedClasses = @(Get-ClientPatchSelectedClassNames -PatchMode $ProductionMode @ClassSelectionArguments -SkipVersionClass)
    $ExpectedProductionClasses = @("api", "dev-config", "title-scene", "channel-main")
    Assert-TestCondition -Condition ($ProductionSelectedClasses.Count -eq $ExpectedProductionClasses.Count -and ($ProductionSelectedClasses -join "|") -ceq ($ExpectedProductionClasses -join "|")) -Message "内嵌个人服务模式没有选择完整正式登录类"
