...
# 此程序在 CN Android 主 DEX 中替换已知 SDK 初始化调用, 并更新 DEX 签名与校验和.
...
# //// 读取 DEX 整数编码 [@x380kkm 2026-08-31] ////
...
# //// 保存 DEX 索引表位置 [@x380kkm 2026-08-31] ////
...
# //// 解析目标类, 方法和调用指令 [@x380kkm 2026-08-31] ////
class DexReader:
...
# //// 替换唯一 SDK 初始化调用并更新 DEX 校验字段 [@x380kkm 2026-08-31] ////
...
# //// /替换唯一 SDK 初始化调用并更新 DEX 校验字段 ////


# //// 生成 DEX 补丁报告 [@x380kkm 2026-08-31] ////
def build_report(
    source: bytes,
    patched: bytes,
    mode: str,
...
# //// 解析参数并写入补丁 DEX [@x380kkm 2026-08-31] ////
def main() -> None:
    parser = argparse.ArgumentParser(description="替换 CN Android 主 DEX 中已知的 SDK 初始化调用.")
    parser.add_argument(
        "--mode",
    ...
    if args.report:
        ...
        args.report.write_text(
            json.dumps(
                build_report(source, patched, args.mode, instruction_offset),
                ensure_ascii=False,
                indent=2,
            )
            + "\n",
...
# //// 运行 DEX 补丁命令 [@x380kkm 2026-08-31] ////
