# audience: internal
# # patch-cn-client
# 此脚本把 CN APK 中的版本地址和 API 端点改为指定服务器, 内嵌个人服务模式启用 dummy SDK 运行路径和原生扩展守卫, 诊断开关添加请求观察, 版本查询, 响应解码和预展开资源补丁, 再重建, 对齐和签名 APK.
...
param(
    ...
    [switch]$EmbeddedPersonalService
)
...
# //// 验证最终 P-code 使用 true 初始化 sdkDummy [@x380kkm 2026-09-02] ////
function Assert-ClientPatchSdkDummyPcode {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$PcodePath)

    $Content = [IO.File]::ReadAllText($PcodePath)
    $Patterns = [ordered]@{
        ScriptInitializer = '(?ms)getlex\s+QName\(PackageNamespace\("pinball\.config\.core"\),"DevConfig"\)\s+push(?<value>true|false)\s+initproperty\s+QName\(PackageNamespace\(""\),"sdkDummy"\)'
        ClassInitializer = '(?ms)findproperty\s+QName\(PackageNamespace\(""\),"sdkDummy"\)\s+push(?<value>true|false)\s+setproperty\s+QName\(PackageNamespace\(""\),"sdkDummy"\)'
    }
    $Initializers = @(
        foreach ($Shape in $Patterns.Keys) {
            foreach ($Match in [regex]::Matches($Content, $Patterns[$Shape])) {
                [pscustomobject]@{ Shape = $Shape; Match = $Match }
            }
        }
    )
    if ($Initializers.Count -ne 1) {
        throw "最终 P-code 的 sdkDummy 初始化结构数量不正确: expected=1 actual=$($Initializers.Count) pcode=$PcodePath"
    }

    $Initializer = $Initializers[0]
    $Value = $Initializer.Match.Groups['value'].Value
    if ($Value -cne 'true') {
        throw "最终 P-code 使用非 true 值初始化 sdkDummy: actual=$Value pcode=$PcodePath"
    }
...
# //// /验证最终 P-code 使用 true 初始化 sdkDummy ////
...
# //// 读取 SDK 登录补丁模式 [@x380kkm 2026-09-02] ////
function Get-ClientPatchSdkLoginMode {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][bool]$EmbeddedPersonalService,
        [Parameter(Mandatory)][bool]$ProbeSdkDummyLoginRequest
    )

    $PatchRuntimeLogin = $EmbeddedPersonalService -or $ProbeSdkDummyLoginRequest
    $Name = if ($EmbeddedPersonalService) {
        if ($ProbeSdkDummyLoginRequest) { 'embedded-personal-service-with-login-request-probe' } else { 'embedded-personal-service' }
    } elseif ($ProbeSdkDummyLoginRequest) {
        'dummy-login-request-probe'
    } else {
        'none'
    }
    ...
    [pscustomobject][ordered]@{
        Name = $Name
        EmbeddedPersonalService = $EmbeddedPersonalService
        PatchApiEndpoint = $true
        PatchSdkDummy = $PatchRuntimeLogin
        PatchChannelMainForSdkDummy = $PatchRuntimeLogin
        PatchDummyLoginRequestProbe = $ProbeSdkDummyLoginRequest
        PatchSdkDummyNativeExtensionGuard = $PatchRuntimeLogin
        PatchRemoteUtilRequestGuard = $PatchRuntimeLogin
        PatchSdkDummyTitleSceneGuard = $PatchRuntimeLogin
    }
}
# //// /读取 SDK 登录补丁模式 ////

# //// 构造外部服务的登录请求探测模式 [@x380kkm 2026-09-02] ////
function Get-ClientPatchSdkDummyLoginRequestProbeMode {
    [CmdletBinding()]
    param([Parameter(Mandatory)][bool]$ProbeSdkDummyLoginRequest)

    Get-ClientPatchSdkLoginMode `
        -EmbeddedPersonalService $false `
        -ProbeSdkDummyLoginRequest $ProbeSdkDummyLoginRequest
}
# //// /构造外部服务的登录请求探测模式 ////
...
if ([bool]$EmbeddedPersonalService) {
    if ($ServerHost -cne "127.0.0.1" -or $Port -ne 17171) {
        throw "内嵌个人服务只允许 127.0.0.1:17171."
    }
} elseif (-not (Test-ClientPatchAndroidServerHost -ServerHost $ServerHost)) {
...
$SdkLoginPatchMode = Get-ClientPatchSdkLoginMode `
    -EmbeddedPersonalService ([bool]$EmbeddedPersonalService) `
...
if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) {
    foreach ($Tool in @(
...
$VerificationPcodeDirectory = Join-Path $WorkingDirectory "ffdec-verify-pcode"
...
try {
    ...
    $JavacVersionResult = if ($VersionUrlPatchEnabled -or $SdkLoginPatchMode.PatchRemoteUtilRequestGuard `
    ...
    $SdkDummyBeforeEvidence = if ($SdkLoginPatchMode.PatchSdkDummy) {
        Get-ClientPatchFileEvidence -Path $DevConfigScript.FullName
    } else {
        $null
    }
    $SdkDummyReplacement = if ($SdkLoginPatchMode.PatchSdkDummy) {
        Set-ClientPatchSdkDummy -ScriptPath $DevConfigScript.FullName
    } else {
        $null
    }
    $SdkDummyNativeExtensionGuardBeforeEvidence = if ($SdkLoginPatchMode.PatchSdkDummyNativeExtensionGuard) {
        Get-ClientPatchFileEvidence -Path $DevConfigScript.FullName
    } else {
        $null
    }
    $SdkDummyNativeExtensionGuardReplacement = if ($SdkLoginPatchMode.PatchSdkDummyNativeExtensionGuard) {
        Set-ClientPatchSdkDummyNativeExtensionGuard -ScriptPath $DevConfigScript.FullName
    } else {
        $null
    }
    $SdkDummyAfterEvidence = if ($SdkLoginPatchMode.PatchSdkDummy) {
        Assert-ClientPatchSdkDummy -ScriptPath $DevConfigScript.FullName | Out-Null
        if ($SdkLoginPatchMode.PatchSdkDummyNativeExtensionGuard) {
    ...
    $TitleSceneBeforeEvidence = if ($SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard) {
        Get-ClientPatchFileEvidence -Path $TitleSceneScript.FullName
    } else {
        $null
    }
    $SdkDummyTitleSceneGuardReplacement = if ($SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard) {
        Set-ClientPatchSdkDummyTitleSceneGuard -ScriptPath $TitleSceneScript.FullName
    } else {
        $null
    }
    $TitleSceneAfterEvidence = if ($SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard) {
        Assert-ClientPatchSdkDummyTitleSceneGuard -ScriptPath $TitleSceneScript.FullName | Out-Null
        Get-ClientPatchFileEvidence -Path $TitleSceneScript.FullName
    } else {
        $null
    }
    $ChannelMainBeforeEvidence = if ($SdkLoginPatchMode.PatchChannelMainForSdkDummy) {
        Get-ClientPatchFileEvidence -Path $ChannelMainScript.FullName
    } else {
        $null
    }
    $SdkDummyRealRemoteBridgeReplacement = if ($SdkLoginPatchMode.PatchChannelMainForSdkDummy) {
        Set-ClientPatchSdkDummyRealRemoteBridge -ScriptPath $ChannelMainScript.FullName
    } else {
        $null
    }
    $ChannelMainAfterEvidence = if ($SdkLoginPatchMode.PatchChannelMainForSdkDummy) {
        Assert-ClientPatchSdkDummyRealRemoteBridge -ScriptPath $ChannelMainScript.FullName
        Get-ClientPatchFileEvidence -Path $ChannelMainScript.FullName
    } else {
        $null
    }
    $ChannelDummyBeforeEvidence = if ($SdkLoginPatchMode.PatchDummyLoginRequestProbe) {
        Get-ClientPatchFileEvidence -Path $ChannelDummyScript.FullName
    } else {
        $null
    }
    $SdkDummyLoginRequestProbeReplacement = if ($SdkLoginPatchMode.PatchDummyLoginRequestProbe) {
        Set-ClientPatchSdkDummyLoginRequestProbe -ScriptPath $ChannelDummyScript.FullName
    } else {
        $null
    }
    $ChannelDummyAfterEvidence = if ($SdkLoginPatchMode.PatchDummyLoginRequestProbe) {
    ...
    $RemoteUtilAbcPatchEvidence = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) {
    ...
    $RemoteUtilAbcPatchVerification = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) {
        Assert-ClientPatchRemoteUtilAbcEvidence -Evidence $RemoteUtilAbcPatchEvidence
    } else {
        $null
    }
    $RemoteUtilAbcPatchEvidenceFile = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) {
    ...
    $SdkDummyPcodeVerification = if ($SdkLoginPatchMode.PatchSdkDummy) {
        New-Item -ItemType Directory -Force -Path $VerificationPcodeDirectory | Out-Null
        Invoke-ClientPatchTool -FilePath $JavaPath -ArgumentList @(
            "-jar", $FfdecJarPath,
            "-onerror", "abort",
            "-selectclass", $DevConfigClassName,
            "-format", "script:pcode",
            "-export", "script", $VerificationPcodeDirectory, $PatchedSwfPath
        ) -LogPath (Join-Path $EvidenceDirectory "ffdec-verify-pcode.log") | Out-Null
        $PcodeFiles = @(Get-ChildItem -LiteralPath $VerificationPcodeDirectory -Recurse -File -Filter "DevConfig.pcode")
        if ($PcodeFiles.Count -ne 1) {
            throw "FFDec 最终 sdkDummy P-code 文件数量不正确: expected=1 actual=$($PcodeFiles.Count) directory=$VerificationPcodeDirectory"
        }
        $Verification = Assert-ClientPatchSdkDummyPcode -PcodePath $PcodeFiles[0].FullName
        [pscustomobject][ordered]@{
            File = Get-ClientPatchFileEvidence -Path $PcodeFiles[0].FullName
    ...
    $SdkDummyVerification = if ($SdkLoginPatchMode.PatchSdkDummy) {
        Assert-ClientPatchSdkDummy -ScriptPath $VerificationDevConfigScript.FullName
    } else {
        $null
    }
    $SdkDummyNativeExtensionGuardVerification = if ($SdkLoginPatchMode.PatchSdkDummyNativeExtensionGuard) {
    ...
    $SdkDummyTitleSceneGuardVerification = if ($SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard) {
    ...
    $SdkDummyTitleSceneLoginGateVerification = if ($SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard) {
        Assert-ClientPatchTitleSceneLoginGateEvidence -ScriptPath $VerificationTitleSceneScript.FullName
    } else {
        $null
    }
    $SdkDummyRealRemoteBridgeVerification = if ($SdkLoginPatchMode.PatchChannelMainForSdkDummy) {
    ...
    $SdkDummyLoginRequestProbeVerification = if ($SdkLoginPatchMode.PatchDummyLoginRequestProbe) {
    ...
    $Manifest = [ordered]@{
        ...
        Target = [ordered]@{
            Mode = if ([bool]$EmbeddedPersonalService) { "embedded-personal-service" } else { "external-server" }
            Host = $ServerHost
            Port = $Port
        ...
        Patch = [ordered]@{
            ...
            SdkLoginMode = $SdkLoginPatchMode.Name
            EmbeddedPersonalService = [bool]$SdkLoginPatchMode.EmbeddedPersonalService
            SdkDummyEnabled = [bool]$SdkLoginPatchMode.PatchSdkDummy
            SdkDummyLoginRequestProbeEnabled = [bool]$SdkLoginPatchMode.PatchDummyLoginRequestProbe
            SdkDummyNativeExtensionGuardEnabled = [bool]$SdkLoginPatchMode.PatchSdkDummyNativeExtensionGuard
            RemoteUtilRequestGuardEnabled = [bool]$SdkLoginPatchMode.PatchRemoteUtilRequestGuard
            ...
            SdkDummyTitleSceneGuardEnabled = [bool]$SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard
            SdkDummyTitleSceneLoginGate = $SdkDummyTitleSceneLoginGateVerification
            SdkDummyLoginRequestProbe = [ordered]@{
                ClassName = $ChannelDummyClassName
                ScriptName = $ChannelDummyScriptName
                Enabled = [bool]$SdkLoginPatchMode.PatchDummyLoginRequestProbe
            ...
            SdkDummy = [ordered]@{
                ...
                Enabled = [bool]$SdkLoginPatchMode.PatchSdkDummy
                ...
                PcodeVerification = $SdkDummyPcodeVerification
            }
            SdkDummyNativeExtensionGuard = [ordered]@{
                Enabled = [bool]$SdkLoginPatchMode.PatchSdkDummyNativeExtensionGuard
                DevConfig = [ordered]@{
                    ClassName = $DevConfigClassName
                    ScriptName = $DevConfigScriptName
            ...
            RemoteUtilRequestGuard = [ordered]@{
                ...
                Enabled = [bool]$SdkLoginPatchMode.PatchRemoteUtilRequestGuard
            ...
            SdkDummyTitleSceneGuard = [ordered]@{
                ClassName = $TitleSceneClassName
                ScriptName = $TitleSceneScriptName
                Enabled = [bool]$SdkLoginPatchMode.PatchSdkDummyTitleSceneGuard
            ...
            SdkDummyRealRemoteBridge = [ordered]@{
                ClassName = $ChannelMainClassName
                ScriptName = $ChannelMainScriptName
                Enabled = [bool]$SdkLoginPatchMode.PatchChannelMainForSdkDummy
        ...
        Tools = [ordered]@{
            ...
            RemoteUtilAbcPatch = [ordered]@{
                Enabled = [bool]$SdkLoginPatchMode.PatchRemoteUtilRequestGuard
                Source = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) { Get-ClientPatchFileEvidence -Path $RemoteUtilAbcPatchSourcePath } else { $null }
                AbcMethodDigestSource = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) { Get-ClientPatchFileEvidence -Path $AbcMethodDigestSourcePath } else { $null }
                MethodDigestSource = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) { Get-ClientPatchFileEvidence -Path $RemoteUtilMethodDigestSourcePath } else { $null }
                FfdecLibrary = if ($SdkLoginPatchMode.PatchRemoteUtilRequestGuard) { Get-ClientPatchFileEvidence -Path $FfdecLibraryJarPath } else { $null }
            }
