...
function Test-ProtocolLabAdbRootRetriesTransientFailure {
    ...
    $Result = & $Module {
        ...
        $RootResult = Invoke-ProtocolLabAdbRootWithRetry -AdbPath "adb.exe" -Serial "emulator-5554" -TimeoutSeconds 6 -MaximumAttempts 2 -RetryDelayMilliseconds 0
        [pscustomobject]@{
...
function Test-ProtocolLabAdbRootReportsFinalFailure {
    ...
    $FailureMessage = & $Module {
        ...
        try {
            Invoke-ProtocolLabAdbRootWithRetry -AdbPath "adb.exe" -Serial "emulator-5554" -TimeoutSeconds 6 -MaximumAttempts 2 -RetryDelayMilliseconds 0
...
function Test-ProtocolLabAdbRootRespectsDeadlineAndCleanupReserve {
    ...
    $Result = & $Module {
        ...
        try {
            Invoke-ProtocolLabAdbRootWithRetry -AdbPath "adb.exe" -Serial "emulator-5554" -TimeoutSeconds 15
            throw "持续失败的 adb root 未抛出错误"
        } catch {
            [pscustomobject]@{
                AttemptCount = $script:RootInvocationCount
...
function Test-ProtocolLabAdbRootRejectsShortTimeout {
    ...
    $Failure = & $Module {
        try {
            Invoke-ProtocolLabAdbRootWithRetry -AdbPath "adb.exe" -Serial "emulator-5554" -TimeoutSeconds 5
            $false
