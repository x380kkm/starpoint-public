...
# 所有运行时产物写入 Git 共享工作区的 artifacts, 源码目录不保存设备状态或捕获内容.

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

# //// 定位 Git 共享工作区 [@x380kkm 2026-09-01] ////
function Resolve-ProtocolLabWorkspaceRoot {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$RepositoryRoot)

    $LASTEXITCODE = 0
    $GitCommonDirectory = @(& git -C $RepositoryRoot rev-parse --git-common-dir 2>$null | Select-Object -First 1)
    $GitCommandSucceeded = $LASTEXITCODE -eq 0
...
function Get-ProtocolLabPaths {
    ...
    $WorkspaceRoot = [IO.Path]::GetFullPath((Resolve-ProtocolLabWorkspaceRoot -RepositoryRoot $RepositoryRoot))
...
# //// 分配进程专用 ADB server 端口 [@x380kkm 2026-09-01] ////
function New-ProtocolLabAdbServerPort {
    [CmdletBinding()]
    param()
...
# //// /分配进程专用 ADB server 端口 ////

# //// 按参数和状态解析 ADB server 端口 [@x380kkm 2026-08-18] ////
function Resolve-ProtocolLabAdbServerPort {
    ...
    if ($null -ne $RequestedPort) {
        if ($RequestedPort -lt 1 -or $RequestedPort -gt 65535 -or $RequestedPort -eq 5037) {
    ...
    if ($null -ne $State -and $null -ne $State.PSObject.Properties["AdbServerPort"]) {
        $StatePort = 0
        if (-not [int]::TryParse([string]$State.AdbServerPort, [ref]$StatePort) -or $StatePort -lt 1 -or $StatePort -gt 65535 -or $StatePort -eq 5037) {
            throw "模拟器状态中的 ADB server 端口无效: $($State.AdbServerPort)"
        }
        return $StatePort
    }

    New-ProtocolLabAdbServerPort
}
# //// /按参数和状态解析 ADB server 端口 ////
...
# //// 在截止时间内执行外部进程 [@x380kkm 2026-09-01] ////
function Invoke-ProtocolLabProcessWithinTimeout {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)]
        [string]$FilePath,
    ...
    try {
        ...
        if (-not $Process.Start()) {
            throw "$Description 进程未启动: $FilePath"
        }
        ...
        if (-not $Completed) {
            $Output += "$Description 在 $TimeoutSeconds 秒后超时."
        }
        if (-not $TerminationCompleted) {
            $Output += "$Description 进程未在 $CleanupTimeoutMilliseconds 毫秒的清理期限内退出."
        }
        if (-not $OutputTasksCompleted) {
            $Output += "$Description 输出未在 $CleanupTimeoutMilliseconds 毫秒的清理期限内关闭."
        }
...
function Invoke-ProtocolLabAdbWithinTimeout {
    ...
    $Arguments = @("-P", $AdbServerPort.ToString([Globalization.CultureInfo]::InvariantCulture), "-s", $Serial) + $CommandArguments
    Invoke-ProtocolLabProcessWithinTimeout -FilePath $AdbPath -Arguments $Arguments -Description "ADB 命令" -TimeoutSeconds $TimeoutSeconds
}
# //// /在截止时间内执行指定模拟器的 ADB 进程 ////

# //// 验证 ADB server 端口与系统默认实例隔离 [@x380kkm 2026-09-01] ////
function Assert-ProtocolLabAdbServerPort {
    [CmdletBinding()]
    param([Parameter(Mandatory)][ValidateRange(1, 65535)][int]$AdbServerPort)

    if ($AdbServerPort -eq 5037) {
        throw "ADB server 端口必须避开系统默认端口 5037."
    }
}
# //// /验证 ADB server 端口与系统默认实例隔离 ////

# //// 验证 ADB 目标是约束的 Android Emulator [@x380kkm 2026-09-01] ////
function Assert-ProtocolLabEmulatorSerial {
    [CmdletBinding()]
    param([Parameter(Mandatory)][string]$Serial)

    if ($Serial -notmatch '^emulator-[0-9]+$') {
        throw "ADB 目标序列号必须是 Android Emulator: $Serial"
    }
}
# //// /验证 ADB 目标是约束的 Android Emulator ////

# //// 验证状态属于无界面专用 Emulator [@x380kkm 2026-09-01] ////
function Assert-ProtocolLabEmulatorState {
    [CmdletBinding()]
    param([Parameter(Mandatory)][object]$State)

    $RequiredProperties = @("Serial", "AdbServerPort", "Headless", "AdbIsolation")
    foreach ($PropertyName in $RequiredProperties) {
        if ($null -eq $State.PSObject.Properties[$PropertyName]) {
            throw "模拟器状态缺少字段: $PropertyName"
        }
    }
    Assert-ProtocolLabEmulatorSerial -Serial ([string]$State.Serial)
    Assert-ProtocolLabAdbServerPort -AdbServerPort ([int]$State.AdbServerPort)
    if (-not [bool]$State.Headless -or [string]$State.AdbIsolation -cne "one-device") {
        throw "模拟器状态不属于无界面专用 AVD: serial=$($State.Serial)"
    }
}
# //// /验证状态属于无界面专用 Emulator ////

# //// 检查专用 ADB server 是否保持监听 [@x380kkm 2026-09-01] ////
function Test-ProtocolLabAdbServerListening {
    [CmdletBinding()]
    param([Parameter(Mandatory)][ValidateRange(1, 65535)][int]$AdbServerPort)

    Assert-ProtocolLabAdbServerPort -AdbServerPort $AdbServerPort
    $Client = [Net.Sockets.TcpClient]::new()
    try {
        $ConnectTask = $Client.ConnectAsync([Net.IPAddress]::Loopback, $AdbServerPort)
...
# //// /检查专用 ADB server 是否保持监听 ////

# //// 执行不绑定设备的 ADB server 命令 [@x380kkm 2026-09-01] ////
function Invoke-ProtocolLabAdbServer {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$AdbPath,
        [Parameter(Mandatory)][string[]]$CommandArguments,
        [Parameter(Mandatory)][ValidateRange(1, 65535)][int]$AdbServerPort,
        [switch]$AllowFailure,
    ...
    Assert-ProtocolLabAdbServerPort -AdbServerPort $AdbServerPort
    $IsStartServer = $CommandArguments.Count -eq 3 -and ($CommandArguments -join " ") -match "--one-device\s+\S+\s+start-server$"
    $IsKillServer = $CommandArguments.Count -eq 1 -and $CommandArguments[0] -eq "kill-server"
    if (-not $IsStartServer -and -not $IsKillServer -and -not (Test-ProtocolLabAdbServerListening -AdbServerPort $AdbServerPort)) {
        if ($AllowFailure) {
            return [pscustomobject]@{ ExitCode = 1; Output = @("专用 ADB server 未在端口 $AdbServerPort 监听") }
        }
        throw "专用 ADB server 未在端口 $AdbServerPort 监听."
    }
    $Result = Invoke-ProtocolLabProcessWithinTimeout -FilePath $AdbPath -Arguments (@("-P", $AdbServerPort.ToString([Globalization.CultureInfo]::InvariantCulture)) + $CommandArguments) -Description "ADB server 命令" -TimeoutSeconds $TimeoutSeconds
    if ($Result.ExitCode -ne 0 -and -not $AllowFailure) {
        throw "ADB server 命令失败, exit=$($Result.ExitCode): adb -P $AdbServerPort $($CommandArguments -join ' ')`n$($Result.Output -join [Environment]::NewLine)"
    }

    $Result
}
# //// /执行不绑定设备的 ADB server 命令 ////

# //// 启动仅允许指定 Emulator 的 ADB server [@x380kkm 2026-09-01] ////
function Start-ProtocolLabAdbServer {
    ...
    Assert-ProtocolLabEmulatorSerial -Serial $Serial
    Assert-ProtocolLabAdbServerPort -AdbServerPort $AdbServerPort
    Invoke-ProtocolLabAdbServer -AdbPath $AdbPath -AdbServerPort $AdbServerPort -CommandArguments @("--one-device", $Serial, "start-server") -TimeoutSeconds $TimeoutSeconds
}
# //// /启动仅允许指定 Emulator 的 ADB server ////

# //// 停止当前进程专用的 ADB server [@x380kkm 2026-09-01] ////
function Stop-ProtocolLabAdbServer {
    ...
    Invoke-ProtocolLabAdbServer -AdbPath $AdbPath -AdbServerPort $AdbServerPort -CommandArguments @("kill-server") -AllowFailure:$AllowFailure -TimeoutSeconds $TimeoutSeconds
}
# //// /停止当前进程专用的 ADB server ////

# //// 读取专用 ADB server 的设备列表 [@x380kkm 2026-09-01] ////
function Get-ProtocolLabAdbDevices {
    ...
    Invoke-ProtocolLabAdbServer -AdbPath $AdbPath -AdbServerPort $AdbServerPort -CommandArguments @("devices", "-l") -AllowFailure:$AllowFailure -TimeoutSeconds $TimeoutSeconds
}
# //// /读取专用 ADB server 的设备列表 ////

# //// 在执行设备命令前验证 Emulator 身份 [@x380kkm 2026-09-01] ////
function Assert-ProtocolLabEmulatorTarget {
    [CmdletBinding()]
    param(
        [Parameter(Mandatory)][string]$AdbPath,
        [Parameter(Mandatory)][string]$Serial,
        [Parameter(Mandatory)][ValidateRange(1, 65535)][int]$AdbServerPort,
        [ValidateRange(1, 30)][int]$CommandTimeoutSeconds = 5
    )

    Assert-ProtocolLabEmulatorSerial -Serial $Serial
    if (-not (Test-ProtocolLabAdbServerListening -AdbServerPort $AdbServerPort)) {
        throw "专用 ADB server 未在端口 $AdbServerPort 监听."
    }
    $StateResult = Invoke-ProtocolLabAdbWithinTimeout -AdbPath $AdbPath -Serial $Serial -CommandArguments @("get-state") -AdbServerPort $AdbServerPort -TimeoutSeconds $CommandTimeoutSeconds
    if ($StateResult.ExitCode -ne 0 -or ($StateResult.Output -join "").Trim() -ne "device") {
        throw "ADB 目标不是可用的 Emulator: serial=$Serial state=$($StateResult.Output -join ' ')"
    }

    $DevicesResult = Get-ProtocolLabAdbDevices -AdbPath $AdbPath -AdbServerPort $AdbServerPort -TimeoutSeconds $CommandTimeoutSeconds
    $DeviceRows = @($DevicesResult.Output | Where-Object { $_ -match '^\S+\s+\S+' -and $_ -notmatch '^List of devices attached' })
    if ($DeviceRows.Count -ne 1 -or $DeviceRows[0] -notmatch "^$([regex]::Escape($Serial))\s+device(?:\s|$)") {
        throw "专用 ADB server 未唯一暴露目标 Emulator: serial=$Serial devices=$($DeviceRows -join '; ')"
    }

    $QemuResult = Invoke-ProtocolLabAdbWithinTimeout -AdbPath $AdbPath -Serial $Serial -CommandArguments @("shell", "getprop", "ro.kernel.qemu") -AdbServerPort $AdbServerPort -TimeoutSeconds $CommandTimeoutSeconds
    if ($QemuResult.ExitCode -ne 0 -or ($QemuResult.Output -join "").Trim() -cne "1") {
        throw "ADB 目标不是 Android Emulator: serial=$Serial ro.kernel.qemu=$($QemuResult.Output -join ' ')"
    }
}
# //// /在执行设备命令前验证 Emulator 身份 ////
...
function Invoke-ProtocolLabAdb {
    ...
    Assert-ProtocolLabAdbServerPort -AdbServerPort $AdbServerPort
    Assert-ProtocolLabEmulatorSerial -Serial $Serial
    $ServerTimeoutSeconds = if ($TimeoutSeconds -gt 0) {
        [Math]::Min($TimeoutSeconds, 15)
    } else {
        15
    }
    Start-ProtocolLabAdbServer -AdbPath $AdbPath -Serial $Serial -AdbServerPort $AdbServerPort -TimeoutSeconds $ServerTimeoutSeconds | Out-Null
    $IsConnectionQuery = $CommandArguments.Count -eq 1 -and $CommandArguments[0] -eq "get-state"
    $IsQemuProbe = $CommandArguments.Count -eq 3 -and ($CommandArguments -join " ") -eq "shell getprop ro.kernel.qemu"
    if (-not $IsConnectionQuery -and -not $IsQemuProbe) {
        $TargetCheckTimeoutSeconds = if ($TimeoutSeconds -gt 0) { [Math]::Min($TimeoutSeconds, 5) } else { 5 }
        try {
            Assert-ProtocolLabEmulatorTarget -AdbPath $AdbPath -Serial $Serial -AdbServerPort $AdbServerPort -CommandTimeoutSeconds $TargetCheckTimeoutSeconds
        } catch {
            if ($AllowFailure) {
...
function Wait-ProtocolLabAndroidBoot {
    ...
    do {
        ...
        $DeviceList = Get-ProtocolLabAdbDevices -AdbPath $AdbPath -AdbServerPort $AdbServerPort -AllowFailure -TimeoutSeconds $CommandTimeoutSeconds
        $TargetDevice = @($DeviceList.Output | Where-Object { $_ -match "^$([regex]::Escape($Serial))\s+device(?:\s|$)" })
        if ($DeviceList.ExitCode -eq 0 -and $TargetDevice.Count -eq 1) {
...
Export-ModuleMember -Function @(
    "Assert-ProtocolLabEmulatorState",
    "Assert-ProtocolLabEmulatorTarget",
